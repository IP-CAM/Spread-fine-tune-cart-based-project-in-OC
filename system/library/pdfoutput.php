...
function hextodec($couleur = "#000000"){
...
function RotateImageOutput($file,$x,$y,$w,$h,$angle)	{
...
function RotateEpsOutput($file,$x,$y,$w,$h,$angle){
		//Image rotated around its upper-left corner
		$this->Rotate($angle,$x,$y);
...
function RotateEpsColorableOutput($file , $x , $y , $w , $h , $angle , $imageColor)
	{
		//Image rotated around its upper-left corner
		$this->Rotate($angle,$x,$y);
		...
		case 'IMG':
			if(isset($attr['SRC']) and (isset($attr['WIDTH']) or isset($attr['HEIGHT']))) {
				...
				if(!isset($attr['HEIGHT']))
					$attr['HEIGHT'] = 0;
				$this->Image($attr['SRC'], $this->GetX(), $this->GetY(), pxtomm($attr['WIDTH']), pxtomm($attr['HEIGHT']));
			}
			break;
		//case 'TR':
		case 'BLOCKQUOTE':
...
function FancyTable($header,$data)
{
	...
	while($row=mysql_fetch_array($data))
	{
       ...
       $jsonEncodedData = $jsonClass->decode($row['designArray']);
		...
		$this->RotateImageOutput($image, 240,95+$dis, 50, 50, 0);		
		$this->Cell($w[2],70,'','LR',0,'C',$fill);
		$this->RotateEpsOutput($Eps, 340,95+$dis, 50, 50, 0);

	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
			  ...
			  $productimage='';
              $productimage=$this->model_sale_order->OrderProductImage($product['product_id']);	
			  	$this->load->model('tool/image');
	
	
			if ($productimage) {
				$productimage = $this->model_tool_image->resize($productimage, $this->config->get('config_image_cart_width'), $this->config->get('config_image_cart_height'));
			} else {
				$productimage = '';
			}
				...
				$checkpdf=$this->model_sale_order->OrderProductPdf($product['product_id'],$this->request->get['order_id']);
				$generate=($checkpdf!='')?'Download Output':'Generate Output';
				if($checkpdf!=''){
				$pdfhref= PDFPRODUCTION.$checkpdf.'.zip';
				$deletefileurl=$this->url->link('rawproduct/generatepdf/Pdfdelete', 'token=' . $this->session->data['token'] . '&pId=' . $product['product_id']. '&orderId=' .  $this->request->get['order_id']. '&zipname=' .  $checkpdf, '');
				}else{
				$pdfhref= $this->url->link('rawproduct/generatepdf/PdfGenerate', 'token=' . $this->session->data['token'] . '&pId=' . $product['product_id']. '&orderId=' .  $this->request->get['order_id'], '');	
				$deletefileurl='';
					}

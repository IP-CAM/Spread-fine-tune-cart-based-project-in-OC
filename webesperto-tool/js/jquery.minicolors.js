...
if(jQuery) (function($) {
		...
		if( target.is('.minicolors-grid') ) {
			picker
				...
				.animate({
                    ...
                    updateFromControl(input, target,event.type);
				});
		} else {
			picker
				.stop(true)
	...
	function updateFromControl(input, target,eventType) {
		...
		// Handle change event
		doChange(input, hex, opacity,eventType);
	...
	function doChange(input, hex, opacity,eventType) {
			...
			if( settings.change ) {
				if( settings.changeDelay ) {
					...
					input.data('minicolors-changeTimeout', setTimeout( function() {
						settings.change.call(input.get(0), hex, opacity,eventType);

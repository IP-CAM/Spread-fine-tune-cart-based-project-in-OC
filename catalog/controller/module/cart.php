	...
	public function index() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				// by nafees==
				$isuserby=$this->cart->checkProductBy($product['product_id']);
				if($isuserby=='0'){
				$producthrefM=$this->url->link('product/product', 'product_id=' . $product['product_id']);
				}else{
				$producthrefM=$this->url->link('designer/product', 'mainProductId=' . $product['product_id']);
					}
				// by nafees==									
			$this->data['products'][] = array(
				'key'      => $product['key'],
